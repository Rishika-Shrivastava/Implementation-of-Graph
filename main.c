...
void deleteNode(int v, graphNode *myGraph) {
    ...
    if(myGraph->vertexArray[v].vertexNo == -1) {
        printf("Node does not exist\n");
    }
    else {
...
int main()
{
	...
	while(ch != 0)
	{
        ...
        switch(sw)
        {
            ...
            case 5:
                    printf("Delete edge from source to destination? \n");
                    scanf("%d", &src);
                    scanf("%d", &dest);
            ...
            case 10:
                    if(select < 1 || select > 4) {
                        ...
                        printf("Enter your fixed source\n");
                        scanf("%d", &nodeNo);

                        shortDist(dist, nodeNo, g);
                    }
